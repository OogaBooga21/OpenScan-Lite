...
// Bluetooth HID
BleKeyboard bleKeyboard("OpenScan-LITE", "Manufacturer", 100);
  ...
  if (currentState == COUNTING && current_picture < total_pictures) {
    ...
    if (pictures_in_layer >= current_layer_p360) {
      ...
      if (current_layer < config.tilt_changes) {
        ...
        current_layer++;
        if (config.spiral) {
          // Calculate new number of pictures for next layer
          current_layer_p360 = round(config.p360 * pow((100.0 - config.shrink) / 100.0, current_layer));
      ...
      // Calculate steps for this layer based on current_layer_p360
      // This ensures a full 360° rotation regardless of number of pictures
    ...
    case MENU:
      ...
      if (sel == LOW && lastSelState == HIGH) {
        if (currentItem == START) {
          ...
          current_layer = 0;
          pictures_in_layer = 0;
          current_layer_p360 = config.p360;  // Start with base number of pictures
...
void updateDisplay() {
  ...
  switch (currentState) {
    ...
    case COUNTING:
      ...
      display.println("Press any key to stop");
      break;
  }
  display.display();
